...
create_approval_from_plan() {
  ...
  jq -nc \
    ...
    '{approvalId:$approvalId, planId:$planId, sessionId:$sessionId, requestedBy:$requestedBy, risk:{score:$score, level:"high", reasons:$reasons}, blastRadius:{files:$files, commandCount:0}, createdAt:$createdAt}'
}
...
set_incident_mode() {
  ...
  curl -sS -X PUT "$BASE_URL/api/incident" \
    ...
    -d "$(jq -nc --argjson enabled "$enabled" --arg by "$REVIEWER" --arg reason "$reason" '{enabled:$enabled,by:$by,reason:$reason}')" >/dev/null
}
...
curl -sS -X POST "$BASE_URL/api/cr/$APPROVAL_A_ID/approve" \
  -H 'Content-Type: application/json' \
  -d "$(jq -nc --arg reviewer "$REVIEWER" --arg comment "Approved by integration test" '{reviewer:$reviewer,comment:$comment}')" >/tmp/aigov-approve-a.json
...
curl -sS -X POST "$BASE_URL/api/cr/$APPROVAL_B_ID/reject" \
  -H 'Content-Type: application/json' \
  -d "$(jq -nc --arg reviewer "$REVIEWER" --arg comment "Denied by integration test" '{reviewer:$reviewer,comment:$comment}')" >/tmp/aigov-reject-b.json
...
curl -sS -X POST "$BASE_URL/api/cr/$APPROVAL_D_ID/approve" \
  -H 'Content-Type: application/json' \
  -d "$(jq -nc --arg reviewer "$REVIEWER" --arg comment "Realtime approval check" '{reviewer:$reviewer,comment:$comment}')" >/tmp/aigov-approve-d.json
sleep 1
if [[ -n "${SSE_PID:-}" ]]; then
...
BLOCK_CODE="$(curl -sS -o /tmp/aigov-incident-block.json -w '%{http_code}' -X POST "$BASE_URL/api/cr/$APPROVAL_C_ID/approve" \
  -H 'Content-Type: application/json' \
  -d "$(jq -nc --arg reviewer "$REVIEWER" --arg comment "Should be blocked" '{reviewer:$reviewer,comment:$comment}')")"
